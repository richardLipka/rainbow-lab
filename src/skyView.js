 ...
 *
 * Measured as displacement from where the button went down, NOT as the length
 * of the path it took. Summing the path was a real bug: a physical mouse
 * emits several one-pixel moves during any ordinary click, six of which
 * totalled more than the threshold, so picking a bow essentially never worked
 * on real hardware while passing a synthetic test that dispatched no moves
 * at all.
 */
const CLICK_SLOP = 6;
...
export function createSkyView(canvas) {
  ...
  /** Everything the projected bow points depend on. */
  function pickKey() {
    return [
  ...
  /**
   * Every drawn bow, sampled at 1-degree rolls and projected to the screen.
   *
   * Brute force, but cached against the camera and the geometry, so it is
   * rebuilt only when the picture actually changes -- never while orbiting,
   * which is the one time it would be rebuilt every frame, and never
   * repeatedly while the pointer merely hovers. That makes it cheap enough
   * to run on pointermove, which is what gives the bows a hover cursor:
   * without one there is no way to tell they can be clicked at all.
   *
   * Testing against the real projection (rather than inverting the camera)
   * means only what is actually on screen can be picked -- the horizon cut
   * and the near plane come along for free.
   */
  function pickPoints() {
    const key = pickKey();
    if (pickCache.key === key) return pickCache.pts;
    const pts = [];
    if (cam) {
      const anti = O.antisolarDirection(state.sunElevation, state.sunAzimuth);
      const dip = O.horizonDipDeg(state.observerHeight);
  ...
  /** The bow nearest a screen position, or null. */
  function pickAt(px, py) {
    let best = null;
    for (const p of pickPoints()) {
      const gap = Math.hypot(p.x - px, p.y - py);
      if (gap < PICK_PX && (!best || gap < best.gap)) {
        best = { gap, k: p.k, lambda: p.lambda, roll: p.roll };
      }
    }
    return best;
  }

  let drag = null; // last pointer position while the button is held
  let down = null; // where the button went down
  let orbiting = false; // slop exceeded: this gesture is an orbit, not a click
  ...
  canvas.addEventListener('pointermove', (e) => {
    if (!drag) {
      ...
      const want = pickAt(e.clientX - r.left, e.clientY - r.top) ? 'pointer' : 'grab';
      if (canvas.style.cursor !== want) canvas.style.cursor = want;
      return;
    }
    if (!orbiting) {
      // Hold the camera still until the pointer has clearly left the press
      // point, so an ordinary click does not also nudge the view.
  ...
  canvas.addEventListener('pointerup', (e) => {
    // Dragging is the primary gesture here, so a release only counts as a
    // pick when the pointer never left the press point. Without the test
    // every orbit would end by re-selecting whatever it landed on.
